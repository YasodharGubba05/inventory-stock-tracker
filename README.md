...
https://inventory-service-rb7q.onrender.com/
...
https://inventory-service-rb7q.onrender.com/docs
...
https://inventory-service-rb7q.onrender.com/api/v1/health
...
Transaction-safe inventory updates using PostgreSQL transactions
...
Optimistic concurrency control for product metadata updates
...
Dockerized deployment on Render with PostgreSQL
...
Fully qualified JDBC/SQLAlchemy connection string (postgresql+asyncpg://...)
...
Verbosity of the structured logger (DEBUG, INFO, WARNING, ERROR)
...
Outputs logs in JSON format (ideal for ELK stack, Datadog, CloudWatch)
...
Lists products (paginated, supports sku and is_active filters)
...
Retrieves products at or below the specified stock threshold
...
Updates metadata (name/SKU) utilizing OCC version check
...
Deactivates if stock movement logs exist; deletes otherwise
...
Registers a stock movement (RESTOCK, SALE, ADJUSTMENT)
...
Paginated historical ledger of stock movements (chronological)
...
/products/{id}/movements/summary



Aggregated statistical summary of movements by type
...
Liveness & readiness probes verifying database connectivity
...
1. Concurrent Sales & Scaling to Multiple Warehouses
...
Note: Live deployment links are provided at the top of this README for quick access.
...
Interactive API Docs (Swagger): https://inventory-service-rb7q.onrender.com/docs



Base URL (JSON Entrypoint): https://inventory-service-rb7q.onrender.com/



System Health Check: https://inventory-service-rb7q.onrender.com/api/v1/health
...
Go to Blueprints and click New Blueprint Instance (or New -> Blueprint).
...
Render will automatically read the render.yaml spec and prompt you to create the services. Click Apply.



Render will automatically provision the PostgreSQL database, build the Docker container, link the DATABASE_URL environment variable, run alembic migrations, seed mock data, and expose the API publicly.
